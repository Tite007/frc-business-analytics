...
export const sampleReportData = {
  ...
  mainPoints: [
    {
      ...
      content: "The iPhone 16 series launch has been Apple's most successful in three years, with the Pro models particularly popular due to enhanced camera capabilities and AI features."
    },
    {
      title: "Services Revenue Growth:",
      content: "Services segment grew 12% YoY to $22.3B, driven by App Store, iCloud, and Apple Music subscriptions reaching new highs."
    },
    {
      title: "China Market Recovery:",
      content: "After several challenging quarters, Apple's China revenue showed signs of recovery, declining only 2% YoY compared to previous double-digit declines."
    }
  ],
  additionalContent: [
  ...
  disclaimer: "This report is for informational purposes only. See important disclosures and risk definitions on the last page.",
  footer: {
...
export function validateReportData(data) {
  ...
  if (!data.recommendation?.rating) errors.push("recommendation.rating is required");
  ...
  if (!Array.isArray(data.mainPoints)) errors.push("mainPoints must be an array");
  if (!data.financialData?.headers) errors.push("financialData.headers is required");
  if (!Array.isArray(data.financialData?.rows)) errors.push("financialData.rows must be an array");
